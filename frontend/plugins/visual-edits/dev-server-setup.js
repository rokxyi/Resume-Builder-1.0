      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="dev@local.sh" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="dev@local.sh" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
